<template>
	<section class="notification-create">
		<form class="notification-create__form form" @submit.prevent="notificationChangeHandler($event)">
			...
			<div class="form__input-container input-container">
				<div class="input-container__wrapper">
					...
					<input id="input_time" type="time" class="form__input" :class="{ _error: error.includes('time') }"
						name="time" :disabled="disabledForm" v-model="notification.time" @input="error = clearVariable(error)" />
				</div>
				<div class="input-container__wrapper">
			...
			<input id="input_body" type="text" name="body" class="form__input" :class="{ _error: error.includes('body') }"
				:disabled="disabledForm" v-model="notification.body" @input="error = clearVariable(error)" />
			...
			<input id="input_repeat-times" type="number" name="repeat_times" class="form__input"
				...
				v-model="notification.repeat_times" @input="error = clearVariable(error)" min="-1" />
			...
			<div class="select-container">
				<select id="input_premium_app" name="premium_app_type" class="form__input"
					:class="{ _error: error.includes('premium_app_type') }" v-model="notification.premium_app_type"
			...
			<input id="input_type" type="text" class="form__input" :class="{ _error: error.includes('text') }" name="type"
				:disabled="disabledForm" v-model="notification.type" @input="error = clearVariable(error)" v-if="false" />
			...
			<div class="select-container">
				<select id="input_lang" name="lang" class="form__input" :class="{ _error: error.includes('lang') }"
					v-model="notification.lang" :disabled="disabledForm">
			...
			<input id="input_days-without-open-app" type="number" name="days_without_open_app" class="form__input"
				:class="{ _error: error.includes('days_without_open_app') }" :disabled="disabledForm"
				v-model="notification.days_without_open_app" @input="error = clearVariable(error)" min="0" />

			<div v-if="notification.premium_app_type === 'not'">
				<label for="input_days-without-sub" class="form__label">
					Дней без подписки
				</label>
				<input id="input_days-without-sub" type="number" name="days_without_subscription" class="form__input"
					:class="{ _error: error.includes('days_without_subscription') }" :disabled="disabledForm"
					v-model="notification.days_without_subscription" @input="error = clearVariable(error)" min="0" />
			</div>
